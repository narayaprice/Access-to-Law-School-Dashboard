...
if view == "Cohort Overview":
    ...
    session_cols = [col for col in attendance_df.columns if col.startswith(('FSG', 'SSG', 'SA')) and not col.endswith('%')]
    ...
    avg_att = att_data.mean().sort_index()
    ...
    st.header("5. Cohort Score Progression")
    ...
    sns.lineplot(data=long_scores, x='Test', y='Score', hue='Name', alpha=0.4, legend=False, ax=ax)
    sns.lineplot(data=avg_scores, x='Test', y='Score', color='black', label='Cohort Avg', linewidth=2, ax=ax)
    ax.set_title('All Fellows - Score Progression Over Time')
    ax.set_ylabel('Score')
    ax.legend(loc='upper left')
    st.pyplot(fig)

    st.header("6. Filtered Score Progression (≥75% Attendance)")
    ...
    fig, ax = plt.subplots(figsize=(12, 5))
    sns.lineplot(data=filtered, x='Test', y='Score', hue='Name', alpha=0.4, legend=False, ax=ax)
    sns.lineplot(data=filtered_avg, x='Test', y='Score', color='green', label='Filtered Avg (≥75%)', linewidth=2, ax=ax)
    ax.set_title('Score Progression for Fellows with ≥75% Attendance')
    ax.set_ylabel('Score')
    ax.legend(loc='upper left')
    st.pyplot(fig)
    ...
    session_cols = [col for col in att_row.columns if col.startswith(('FSG', 'SSG', 'SA')) and not col.endswith('%')]
    att_vals = att_row[session_cols].T.reset_index()
